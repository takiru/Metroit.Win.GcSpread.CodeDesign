...
namespace Metroit.Win.GcSpread.CodeDesign.Json.CellTypeConverters
{
    ...
    internal class ButtonCellTypeConverter : BaseCellTypeConverter
    {
        ...
        protected override void SerializeProp(JObject propsObj, string[] includeProps)
        {
            ...
            if (includeProps == null || includeProps.Any(x => x.Contains(nameof(ButtonCellType.PictureZoomEffect))))
            {
                propsObj.Add(new JProperty(nameof(ButtonCellType.PictureZoomEffect), c.PictureZoomEffect));
            }
        ...
        protected override void DeserializeProp(KeyValuePair<string, JToken> prop)
        {
            ...
            if (string.Compare(prop.Key, nameof(ButtonCellType.PictureZoomEffect), true) == 0)
            {
                c.PictureZoomEffect = prop.Value.ToObject<bool>();
                return;
            }
